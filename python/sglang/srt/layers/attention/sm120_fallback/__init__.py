...
This module bypasses DeepGEMM's ``get_paged_mqa_logits_metadata`` /
``fp8_paged_mqa_logits`` on sm_120 hardware, which DeepGEMM does not support
(see https://github.com/sgl-project/sglang/issues/23657 and
https://github.com/deepseek-ai/DeepGEMM/issues/236). The Lightning Indexer's
metadata helper is small enough to run on the host (a few microseconds for
typical batch shapes), and the FP8 paged MQA logits kernel itself already has
an in-tree TileLang implementation at
``sglang.srt.layers.attention.nsa.tilelang_kernel.tilelang_fp8_paged_mqa_logits``
that is correct on sm_120.

This file is the dispatch glue (poc-16 Phase 2A T2A.1). No new GPU code lives
here; the math is either pure-Python (metadata helper) or a thin shim that
adapts the call-site shape conventions to the existing TileLang wrapper.
...
- :func:`tilelang_fp8_paged_mqa_logits_sm120`: shape-adapter shim that
  forwards into the upstream TileLang kernel.
"""
