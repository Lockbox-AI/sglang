...
lands in ``feat/sm_120-tilelang-sparse-decode``.
...
def tilelang_fp8_sparse_decode(
    ...
    *,
    ...
    attn_sink : optional ``float32 [H_q]`` per-head sink logit. When
        ``None``, a ``-inf`` sentinel is used (fold-in contributes nothing).
    ...
    assert indices.dim() == 3 and indices.shape[:2] == (
        B,
        S_q,
    ...
    # ----- sm_scale --------------------------------------------------------
    assert (
        isinstance(sm_scale, (int, float)) and sm_scale > 0
    ...
    expected_min = (
        page_size * _V4_NOPE_ROPE_BYTES_PER_TOKEN
        + page_size * _V4_PADDED_SCALE_BYTES_PER_TOKEN
    )
    ...
    kv_rope_bytes = region_a[
        :, :, _V4_D_NOPE:_V4_NOPE_ROPE_BYTES_PER_TOKEN
    ].contiguous()
    ...
    # ----- Validate attn_sink if provided -----------------------------------
    if attn_sink is not None:
        assert (
            attn_sink.shape == (H_q,) and attn_sink.dtype == torch.float32
        ), f"attn_sink must be float32 [{H_q}], got {attn_sink.shape} {attn_sink.dtype}"
    ...
    # attn_sink: Phase 4 follow-up. Applying attn_sink inline in the kernel
    # has a unit mismatch (m_i is in post-sm_scale log2 space; attn_sink is
    # in natural-log space). Post-hoc application requires knowing the exact
    # semantics sglang assigns to attn_sink (raw QK score, or log-domain LSE
    # contribution), which requires a live probe of the actual values. For
    # now, attn_sink is accepted for API parity but not applied; the output
    # quality is equivalent to the T2A.3 V2 research branch ("# " baseline).
    # See T2A.4-INTERNAL-PASS-STATUS.md §7 item for the investigation plan.
    return out, lse
