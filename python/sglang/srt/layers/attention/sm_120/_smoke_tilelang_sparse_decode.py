...
Run from the fork's ``python/`` directory::
    ...
    python -m sglang.srt.layers.attention.sm_120._smoke_tilelang_sparse_decode
"""
...
def _build_synthetic_inputs(
    ...
    q_nope = (
        torch.randn(
            B, s_q, H_q, _V4_D_NOPE, dtype=torch.bfloat16, device="cuda", generator=g
        )
        * 0.1
    )
    q_rope = (
        torch.randn(
            B, s_q, H_q, _V4_D_ROPE, dtype=torch.bfloat16, device="cuda", generator=g
        )
        * 0.5
    )
    ...
    nope_bf16 = (
        torch.randn(
            n_total, _V4_D_NOPE, dtype=torch.bfloat16, device="cuda", generator=g
        )
        * 0.1
    )
    ...
    rope_bf16 = (
        torch.randn(
            n_total, _V4_D_ROPE, dtype=torch.bfloat16, device="cuda", generator=g
        )
        * 0.05
    )
    ...
    region_a[:, :, _V4_D_NOPE:_V4_NOPE_ROPE_BYTES_PER_TOKEN].contiguous().view(
        ...
        rope_bf16.view(num_pages, page_size, _V4_D_ROPE)
        .contiguous()
        .view(torch.bfloat16)
    )
...
def main() -> int:
    ...
    # ---- Liveness gates --------------------------------------------------
    assert isinstance(out, torch.Tensor) and isinstance(
        lse, torch.Tensor
