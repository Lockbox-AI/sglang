...
_PAGE_BLOCK_SIZE = 64
_NUM_BLOCKS = 8
_BYTES_PER_TOKEN = 512 + 4 * 4 + 64 * 2  # = 656
...
def test_q_d_qk_must_be_576() -> None:
    """MODEL1 (d_qk=512) is K2b territory; V32 path rejects."""
    kw = _make_valid_inputs()
    kw["q"] = torch.zeros((_B, _S_Q, _H_Q, 512), dtype=torch.bfloat16)
...
def test_k_cache_bytes_per_token_must_match_v32() -> None:
    """V32: 512 (FP8 NoPE) + 16 (4 fp32 scales) + 128 (64 bf16 RoPE) = 656."""
    kw = _make_valid_inputs()
    kw["k_cache"] = torch.zeros(
